...
class PlayerP : public BoxCollider{
public:
    bool onGround;
    float xVelo,yVelo;
    PlayerP(sf::Vector2f position,float speed) {
        setPosition(position);
        this ->speed=speed;
    }
    void update(const sf::Time &elapsed,vector<BoxCollider> colliders){
        ...
        if(sf::Keyboard::isKeyPressed(sf::Keyboard::D))
            ...
            xVelo-=speed;
        if(sf::Keyboard::isKeyPressed(sf::Keyboard::Space) && onGround) yVelo = -900;
        yVelo += 1300 * elapsed.asSeconds();


        float x  = xVelo * elapsed.asSeconds();
        float  y  = yVelo*elapsed.asSeconds();

        onGround = false;
        for ( BoxCollider c : colliders){

            sf::Vector2f xd = (c.getPosition())  - (getPosition()+ sf::Vector2f(x,0) );
            sf::Vector2f yd = (c.getPosition())  - (getPosition()+ sf::Vector2f(0,y) );

            if( (abs(xd.x) < ( width/2.0 + c.width / 2.0)) && (abs(xd.y) < (c.height/2.0 + height/2.0)))

                    x =0;

            if( (abs(yd.x) < ( width/2.0 + c.width / 2.0)) && (abs(yd.y) < (c.height/2.0 + height/2.0)))

            {
                if (yVelo > 0)onGround = true;yVelo=0;y=0;
            }
        }
...
void FinalAssigment6(){
   ...
   sf::Texture wall = loadTexture("wall.jpg");
   //1134x1134
   sf::Texture grass = loadTexture("background.png");
   //277x379d
   sf::Texture p = loadTexture("player.png");

   PlayerP player (sf::Vector2f(0,-100),350);
   ...
   player.spriteTrans.y *= 379/277.0;
       ...
       sf::Sprite bg;
       bg.setTexture(grass);
       bg.setScale(w.y/533.0,w.y/533.0);
       //sprite.setPosition(w.x / (float)mapS * x,  w.y / (float)mapS * y);
    ...
    for (int i=0;i<n;i++){
        //BoxCollider a(wall,sf::Vector2f(w.x/wallS,w.x/wallS));
        BoxCollider a;
      //  a.setTexture(w,wall,425,(rand() % 60) / 10 + 4,425,25);
         a.setTexture(w,wall,425,n,425,25);
        a.setPosition(i*w.x /n,w.y * (rand() % 100) / 100);
        map.push_back(a);
    }

    BoxCollider a;
    a.setTexture(w,wall,425,1,425,15);
    a.setPosition(w.x/2.0,w.y - 10);
    a.height = 10;
    a.width = w.x*3;
